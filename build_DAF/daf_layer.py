    ...
    def get_performances(self, X, y):
        ...
        for idx, (key, estimator) in enumerate(self.estimators_.items()):
            ...
            if idx == 0:
                ...
                performance1.append(accuracy_score(y, y_pred))
                estimaiors_rf.append(estimator)
            elif idx == 1:
                y_pred = estimator.predict(X)
                y_pred = np.argmax(y_pred, axis=1)
                performance1.append(accuracy_score(y, y_pred))
                ...
                y_pred = estimator.predict(X)
                y_pred = np.argmax(y_pred, axis=1)
                performance2.append(accuracy_score(y, y_pred))
                estimaiors_erf.append(estimator)
            elif idx == 3:
                y_pred = estimator.predict(X)
                y_pred = np.argmax(y_pred, axis=1)
                performance2.append(accuracy_score(y, y_pred))
        ...
        return performance1, performance2, estimaiors_rf, estimaiors_erf
...
class ClassificationCascadeLayer(BaseCascadeLayer, ClassifierMixin):
    ...
    def fit_transform(self, X, y, sample_weight=None):
        ...
        y_pred = np.argmax(self.oob_decision_function_, axis=1)
