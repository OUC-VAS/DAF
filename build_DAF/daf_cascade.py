...
class BaseCascadeForest(BaseEstimator, metaclass=ABCMeta):
    ...
    def get_batch_by_index(self, idx_array, file_path, label):
        X_batch = []
        for idx in idx_array:
            sample_data = np.load(file_path[idx])
            X_batch.append(sample_data)
        X_batch = np.array(X_batch)
        y_batch = np.full(len(idx_array), label)

        return X_batch, y_batch


    def _sample_subset_indices(self,
                               idx_pool_real,
                               idx_pool_fake,
                               weights_real,
                               weights_fake,
                               p):
        """
        Sample a subset from real and fake samples according to the current weights:

        - Total subset size = p * (N_real + N_fake)

        Returns:
        subset_real_idx: indices of selected real samples for this layer
        subset_fake_idx: indices of selected fake samples for this layer
        rest_real_idx:   indices of remaining real samples not selected
        rest_fake_idx:   indices of remaining fake samples not selected
        """
        N_real = len(idx_pool_real)
        N_fake = len(idx_pool_fake)
        N_all = N_real + N_fake

        # Total subset size
        subset_size = max(2, int(p * N_all))
        subset_size = min(subset_size, N_all)
        if subset_size % 2 == 1:
            subset_size -= 1
        half = subset_size // 2
        if half == 0:
            return idx_pool_real.copy(), idx_pool_fake.copy(), \
                np.array([], dtype=int), np.array([], dtype=int)

        w_real = weights_real[idx_pool_real]
        w_fake = weights_fake[idx_pool_fake]
        ...
        p_real = w_real / w_real.sum()
        p_fake = w_fake / w_fake.sum()

        n_real = min(half, N_real)
        n_fake = min(half, N_fake)

        subset_real_idx = np.random.choice(
            idx_pool_real, size=n_real, replace=False, p=p_real
        )
        subset_fake_idx = np.random.choice(
            idx_pool_fake, size=n_fake, replace=False, p=p_fake
        )

        rest_real_idx = np.setdiff1d(idx_pool_real, subset_real_idx)
        rest_fake_idx = np.setdiff1d(idx_pool_fake, subset_fake_idx)

        return subset_real_idx, subset_fake_idx, rest_real_idx, rest_fake_idx

    def _log_layer_subset(self, layer_idx,
                          subset_real_idx, subset_fake_idx,
                          rest_real_idx, rest_fake_idx):
        """
        Record the indices of samples selected for this layer, as well as those not selected.
        """
        if not hasattr(self, "layer_subset_log_"):
            self.layer_subset_log_ = []

        self.layer_subset_log_.append({
            "layer": layer_idx,
            "subset_real_idx": subset_real_idx.tolist(),
            "subset_fake_idx": subset_fake_idx.tolist(),
            "rest_real_idx": rest_real_idx.tolist(),
            "rest_fake_idx": rest_fake_idx.tolist(),
        })

    def _update_weights_with_rest(self,
                                  model,
                                  real_path,
                                  fake_path,
                                  rest_real_idx,
                                  ...
                                  weights_fake,
                                  w=1.5):
        for idx in rest_real_idx:
            X_rest, y_rest = self.get_feature_by_index(real_path[idx], 0)
        ...
        for idx in rest_fake_idx:
            X_rest, y_rest = self.get_feature_by_index(fake_path[idx], 1)
            X_rest = X_rest.reshape(1, -1)
            y_pred = model.predict(X_rest)
            if y_pred != y_rest:
                weights_fake[idx] *= w
        ...
        weights_real[:] = np.clip(weights_real, 1e-6, 1e6)
        weights_fake[:] = np.clip(weights_fake, 1e-6, 1e6)

    def fit_v(self, real_path, fake_path, weights, p=0.1, w=1.5, v=3, layer_num=2, iter_num=10, model_temp=None,
              model_sample_weight=None):
        N_real = len(real_path)
        N_fake = len(fake_path)
        weights = np.asarray(weights, dtype=float)
        assert weights.shape[0] == N_real + N_fake
        weights_real = weights[:N_real].copy()
        weights_fake = weights[N_real:].copy()
        idx_pool_real = np.arange(N_real)
        idx_pool_fake = np.arange(N_fake)
        idx_pool_real_train, idx_pool_real_val = train_test_split(idx_pool_real, test_size=0.1, random_state=42)
        idx_pool_fake_train, idx_pool_fake_val = train_test_split(idx_pool_fake, test_size=0.1, random_state=42)

        # ========= Layer 0: Initial subset sampling based on weights =========
        subset_real_idx, subset_fake_idx, rest_real_idx, rest_fake_idx = self._sample_subset_indices(
            idx_pool_real=idx_pool_real_train,
            idx_pool_fake=idx_pool_fake_train,
            weights_real=weights_real,
            weights_fake=weights_fake,
            p=p,
        )

        # ========= Record the samples utilized in this layer =========
        self._log_layer_subset(
            layer_idx=0,
            subset_real_idx=subset_real_idx,
            subset_fake_idx=subset_fake_idx,
            rest_real_idx=rest_real_idx,
            rest_fake_idx=rest_fake_idx,
        )

        # ========= Load the "subset" features for training at this layer =========
        X_real, y_real = self.get_batch_by_index(subset_real_idx, real_path, 0)
        X_fake, y_fake = self.get_batch_by_index(subset_fake_idx, fake_path, 1)
        X_batch = np.concatenate([X_real, X_fake], axis=0)
        y_batch = np.concatenate([y_real, y_fake], axis=0)
        self._check_input(X_batch, y_batch)

        T_num = 1
        models = []
        if self.verbose > 0:
            msg = "{} fitting forest{}..."
            print(msg.format(_utils.ctime(), 0))
        model = CascadeForestClassifier(max_layers=layer_num, random_state=42)
        model.fit(X_batch, y_batch)
        models.append(model)
        self._update_weights_with_rest(
            model=model,
            real_path=real_path,
            fake_path=fake_path,
            rest_real_idx=rest_real_idx,
        ...
        for i in range(iter_num):
            if self.verbose > 0:
                msg = "{} iter --- {}"
                print(msg.format(_utils.ctime(), i + 1))
            while (T_num < v):
                if self.verbose > 0:
                    msg = "{} fitting forest{}..."
                    print(msg.format(_utils.ctime(), T_num))
                if i > 0:
                    idx_pool_real_train, idx_pool_real_val = train_test_split(idx_pool_real, test_size=0.1,
                                                                              random_state=42)
                    idx_pool_fake_train, idx_pool_fake_val = train_test_split(idx_pool_fake, test_size=0.1,
                                                                              random_state=42)
                subset_real_idx, subset_fake_idx, rest_real_idx, rest_fake_idx = self._sample_subset_indices(
                    idx_pool_real=idx_pool_real_train,
                    idx_pool_fake=idx_pool_fake_train,
                ...
                T_num += 1
            X_remaining_real, y_remaining_real = self.get_batch_by_index(idx_pool_real_val, real_path, 0)
            X_remaining_fake, y_remaining_fake = self.get_batch_by_index(idx_pool_fake_val, fake_path, 1)
            X_remaining = np.concatenate([X_remaining_real, X_remaining_fake], axis=0)
            y_remaining = np.concatenate([y_remaining_real, y_remaining_fake], axis=0)
            self._check_input(X_remaining, y_remaining)
            if self.verbose > 0:
                msg = "{} Get {} forests! Start merging！"
                print(msg.format(_utils.ctime(), v))
            for order in range(layer_num):
                estimaiors_v = []
                performances1 = []
                performances2 = []
                estimaiors_rf = []
                ...
                if order == 0:
                    for j in range(v):
                        model = models[j]
                        layer = model._get_layer(order)
                        binner_ = model._get_binner(0)
                        X_remaining_binned = model._bin_data(binner_, X_remaining, is_training_data=False)
                        performance1, performance2, estimaior_rf, estimaior_erf = layer.get_performances(
                            X_remaining_binned, y_remaining)
                        performances1.extend(performance1)
                        performances2.extend(performance2)
                        estimaiors_rf.extend(estimaior_rf)
                        estimaiors_erf.extend(estimaior_erf)
                    best_idx1 = np.argmax(performances1)
                    estimaiors_v.append(estimaiors_rf[best_idx1])
                    tmp1 = performances1.copy()
                    tmp1[best_idx1] = -np.inf
                    next_idx1 = np.argmax(tmp1)
                    estimaiors_v.append(estimaiors_rf[next_idx1])
                    best_idx2 = np.argmax(performances2)
                    estimaiors_v.append(estimaiors_erf[best_idx2])
                    tmp2 = performances2.copy()
                    tmp2[best_idx2] = -np.inf
                    next_idx2 = np.argmax(tmp2)
                    estimaiors_v.append(estimaiors_erf[next_idx2])
                    if self.verbose > 0:
                        msg = "{} Layer {} merged"
                        print(msg.format(_utils.ctime(), 0))
                else:
                    for j in range(v):
                        model = models[j]
                        for _layer in range(order + 1):
                            layer = model._get_layer(_layer)
                            if _layer == 0:
                                binner_ = model._get_binner(0)
                                X_remaining_binned = model._bin_data(binner_, X_remaining, is_training_data=False)
                                X_aug_layer = layer.transform(X_remaining_binned)
                                X_remaining_binned = _utils.init_array(X_remaining_binned, self.n_aug_features_)
                            elif _layer < order:
                                binner_ = model._get_binner(_layer)
                                X_aug_layer = self._bin_data(
                                    binner_, X_aug_layer, is_training_data=False
                                )
                                X_remaining_binned = _utils.merge_array(
                                    X_remaining_binned, X_aug_layer, self.n_features_
                                )
                                X_aug_layer = layer.transform(X_remaining_binned)
                            else:
                                binner_ = model._get_binner(_layer)
                    ...
                    best_idx1 = np.argmax(performances1)
                    estimaiors_v.append(estimaiors_rf[best_idx1])
                    tmp = performances1.copy()
                    tmp[best_idx1] = -np.inf
                    next_idx1 = np.argmax(tmp)
                    estimaiors_v.append(estimaiors_rf[next_idx1])
                    best_idx1 = np.argmax(performances2)
                    estimaiors_v.append(estimaiors_erf[best_idx1])
                    tmp = performances2.copy()
                    tmp[best_idx1] = -np.inf
                    next_idx1 = np.argmax(tmp)
                    estimaiors_v.append(estimaiors_erf[next_idx1])
                    if self.verbose > 0:
                        msg = "{} Layer {} merged"
                        print(msg.format(_utils.ctime(), order))
                layer_temp = model_temp._get_layer(order)
                key = "{}-{}-{}".format(order, 0, "rf")
                layer_temp.estimators_.update({key: estimaiors_v[0]})
                key = "{}-{}-{}".format(order, 1, "rf")
                layer_temp.estimators_.update({key: estimaiors_v[1]})
                key = "{}-{}-{}".format(order, 0, "erf")
                layer_temp.estimators_.update({key: estimaiors_v[2]})
                key = "{}-{}-{}".format(order, 1, "erf")
                layer_temp.estimators_.update({key: estimaiors_v[3]})
                layer_key = "layer_{}".format(order)
                model_temp.layers_.update({layer_key: layer_temp})
            if i == 0:
                y_pred = model_temp.predict(X_remaining)
                pivot = accuracy_score(y_remaining, y_pred)
                if self.verbose > 0:
                    msg = "{} Performance of iter{}: {}"
                    print(msg.format(_utils.ctime(), 1, pivot))
            else:
                y_pred = model_temp.predict(X_remaining)
                new_pivot = accuracy_score(y_remaining, y_pred)
                if self.verbose > 0:
                    msg = "{} Performance of iter{}: {}"
                    print(msg.format(_utils.ctime(), i + 1, new_pivot))
                if not self._if_improved(new_pivot, pivot, self.delta) and i >= 3:
                    if self.verbose > 0:
            ...
            T_num = 1
            if self.verbose > 0:
                msg = "{} Get merged forest!"
                print(msg.format(_utils.ctime()))
        if i + 1 == iter_num and self.verbose > 0:
            msg = "{} Reaching the maximum number of iter: {}"
            print(msg.format(_utils.ctime(), iter_num))
